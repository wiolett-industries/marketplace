...
export const DEFAULT_RESPONSE_MODEL = 'gpt-5.6-luna';
...
export function updateYamlFile(configPath: string, updates: YamlUpdate[]): void {
  ...
  for (const update of updates) {
    if (update.type === 'set') document.setIn(update.path, update.value);
    else document.deleteIn(update.path);
  }
