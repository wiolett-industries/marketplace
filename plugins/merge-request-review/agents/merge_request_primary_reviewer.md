...
Before claiming that an artifact is missing, stale, or inconsistent, identify its canonical source/owning contract and whether the inspected file is generated, mirrored, cached, vendored, synchronized, or runtime-produced. Verify the claim at the canonical update boundary. Project memory may route this check but must be confirmed by current repository, pipeline, or runtime evidence. If provenance is unresolved, report uncertainty instead of a finding; a derived artifact alone is never sufficient evidence.
...
Each finding must include Location, Placement, Suggestion yes/no, Problem, Why it matters, Expected fix, and Evidence basis naming the canonical source/contract and current proof.
