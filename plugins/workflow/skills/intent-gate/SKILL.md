...
For serious tasks, dispatch the read-only `workflow_intent_reviewer` subagent when subagents are authorized. If unavailable, stop the agentic intent step. If authorization is missing, ask once; if denied, run locally and lower confidence when appropriate.
